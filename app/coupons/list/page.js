...
import styles from './list-coupons.module.css';
...
export default function SubscriptionsListPage() {
  ...
  const [filters, setFilters] = useState({
    type: 'coupons', // Focar apenas em cupons por padrão
    discountValue: 'all',
    couponCode: 'all',
    category: 'all',
    product: 'all',
  ...
  const handleClearFilters = () => {
    const clearedFilters = {
      type: 'coupons', // Manter foco em cupons
      discountValue: 'all',
      couponCode: 'all',
      category: 'all',
      product: 'all',
  ...
  // Renderizar badge do cupom
  const renderCouponBadge = (subscription) => {
    const type = getCouponType(subscription);

    const badgeConfig = {
      full: { text: 'Desconto Total', className: styles.couponBadgeFull },
      partial: { text: 'Desconto Parcial', className: styles.couponBadgePartial },
      none: { text: 'Sem Cupom', className: styles.couponBadgeNone }
    };

    const config = badgeConfig[type];
    return (
      <span className={`${styles.couponBadge} ${config.className}`}>
        {config.text}
      </span>
    );
  };
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = allSubscriptions.map(sub => ({
        ...
        'CRM': sub.personalInfo?.crm || 'N/A',
        'Estado CRM': sub.personalInfo?.stateCrm || 'N/A',
        ...
        'Status Pagamento': sub.status?.payment || 'N/A',
  ...
  return (
    <AuthGuard>
      ...
      <header className={styles.header}>
        <h1 className={styles.headerTitle}>Gestão de Cupons de Desconto</h1>
        <p className={styles.headerSubtitle}>
          Visualize e analise o uso de cupons por produto e seus respectivos descontos aplicados
        </p>
      </header>
      ...
      {stats && (
        <div className={styles.statsGrid}>
          <div className={`${styles.statCard} ${styles.statTotal}`}>
            ...
            <div className={styles.statLabel}>Cupons Utilizados</div>
          </div>
          <div className={`${styles.statCard} ${styles.statFullDiscount}`}>
            <div className={styles.statNumber}>
              {stats.fullDiscountCount}/{stats.fullDiscountOriginal || 0}
            </div>
            <div className={styles.statLabel}>Desconto Total (100%)</div>
          </div>
          <div className={`${styles.statCard} ${styles.statPartialDiscount}`}>
            <div className={styles.statNumber}>
              {stats.partialDiscountCount}/{stats.partialDiscountOriginal || 0}
            </div>
            <div className={styles.statLabel}>Desconto Parcial</div>
          </div>
          <div className={`${styles.statCard} ${styles.statTotalDiscount}`}>
            <div className={styles.statNumber}>{formatCurrency(stats.totalDiscountAmount)}</div>
            <div className={styles.statLabel}>Total em Descontos</div>
          </div>
        </div>
      )}
        ...
        <section className={styles.tableSection}>
          <div className={styles.tableHeader}>
            <h3 className={styles.tableTitle}>
              Registros com Cupons ({pagination.total} {pagination.total === 1 ? 'registro' : 'registros'})
            </h3>
          </div>
          ...
          {subscriptions.length > 0 ? (
                      ...
                      <tr key={subscription.id}>
                        ...
                        <td>
                          <span className={`${styles.statusBadge} ${
                            subscription.status === 'CONFIRMED' ? styles.statusConfirmed :
            ...
            <div className={styles.emptyState}>
              ...
              <p className={styles.emptyStateText}>
                Não há registros com cupons que correspondam aos filtros aplicados.
              </p>
            </div>
          )}
        </section>
