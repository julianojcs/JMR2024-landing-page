  ...
  try {
    ...
    allSubscriptionsForTotal.forEach(sub => {
      ...
      try {
        ...
        console.error('❌ Erro ao expandir inscrição (total):', error, baseSubscription.id);
        // Adicionar um registro simplificado em caso de erro
        allExpandedForTotal.push({
          ...baseSubscription,
          currentProduct: null,
          productType: 'error',
          productTitle: 'Erro ao processar',
          value: baseSubscription.value || 0,
          finalValue: baseSubscription.finalValue || 0
        });
      }
    });
    ...
    function extractPriceValue(priceInput) {
      // Casos básicos
      if (!priceInput) return 0;
      if (priceInput === 0) return 0;
      ...
      // Se é string, processa normalmente
      if (typeof priceInput === 'string') {
        // Remove "R$", espaços, pontos e vírgulas, depois converte para número
        const cleanPrice = priceInput.replace(/[R$\s\.,]/g, '');
        const numValue = parseInt(cleanPrice) || 0;
        return numValue;
      }

      // Se é objeto, tenta acessar propriedades comuns
      if (typeof priceInput === 'object' && priceInput !== null) {
        // Casos comuns: { value: "R$ 355" }, { amount: 355 }, { price: "355" }
        ...
        for (const val of possibleValues) {
          if (val != null) {
            return extractPriceValue(val); // Recursão para processar o valor encontrado
          }
        }
      ...
      // Para outros tipos, tenta converter para string e processar
      try {
        const stringValue = String(priceInput);
        const cleanPrice = stringValue.replace(/[R$\s\.,]/g, '');
        const numValue = parseInt(cleanPrice) || 0;
        return numValue;
      } catch (error) {
        console.error('❌ extractPriceValue: erro ao processar:', error, priceInput);
        return 0;
      }
    }
    ...
    function expandSubscriptionByProducts(subscription) {
      try {
        ...
        console.error('❌ Erro ao expandir produtos da inscrição:', subscription.id || 'ID desconhecido', error.message);

        // Retornar um registro simplificado em caso de erro
        return [{
          ...subscription,
          currentProduct: null,
          productType: 'error',
          productTitle: 'Erro ao processar produtos',
          value: subscription.value || 0,
          finalValue: subscription.finalValue || 0
        }];
      }
    }
    ...
    subscriptions.forEach(sub => {
      ...
      try {
        ...
        console.error('❌ Erro ao expandir inscrição (página atual):', error, baseSubscription.id);
        // Adicionar um registro simplificado em caso de erro
